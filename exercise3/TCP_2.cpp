...
SC_MODULE(Producer) {
    // Multi-port output, can connect to multiple FIFOs
...
void Producer::producer_thread() {
    ...
    while (true) {
        ...
        std::cout << "Producer: Packet sent at " << sc_time_stamp()
            ...
            << " to " << out.size() << " consumers" << std::endl;
    }
}
...
SC_MODULE(TopLevel) {
    ...
    Consumer consumer1;
    Consumer consumer2;
    sc_fifo<TCPHeader> fifo_channel1;
    sc_fifo<TCPHeader> fifo_channel2;

    SC_CTOR(TopLevel) :
        producer("Producer"),
        consumer1("Consumer1"),
        consumer2("Consumer2"),
        fifo_channel1("FIFO1", 16),
        fifo_channel2("FIFO2", 16) {

        producer.out(fifo_channel1);
        producer.out(fifo_channel2);

        consumer1.in_port(fifo_channel1);
        consumer2.in_port(fifo_channel2);
    }
};
